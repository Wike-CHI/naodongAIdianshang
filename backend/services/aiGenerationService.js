...
const buildFastApiPayload = (body, files, catalogConfig) => {
  ...
  const placeholderValues = {
    product_description: options.product_description || '时尚服饰展示',
    fabric_description: options.fabric_description || '高品质面料',
    accessory_type: options.accessory_type || '时尚配饰',
    pose_description: options.pose_description || '自然动态姿态',
    shoe_description: options.shoe_description || '潮流鞋靴',
    scene_description: options.scene_description || '高级商业背景',
    target_color: options.target_color || '目标色彩'
  };

  const basePrompt = body.prompt && body.prompt.trim().length > 0
    ? body.prompt
    : catalogConfig.promptTemplate;
